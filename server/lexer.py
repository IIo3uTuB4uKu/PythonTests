...
from colors import colors_editor
...
class NeutronLexer(QsciLexerCustom):
    ...
    def __init__(self, language_name, editor, theme=None, defaults: DefaultConfig = None):
        ...
        if defaults is None:
            ...
            defaults["color"] = colors_editor[self.colors_num_editor][1]
            defaults["paper"] = colors_editor[self.colors_num_editor][0]
        ...
        for clr in colors:
            ...
            for k, v in clr[name].items():
                if k == "color":
                    ...
                    self.setPaper(QColor(colors_editor[self.colors_num_editor][0]), getattr(self, name.upper()))
                elif k == "font":
                    try:
                        self.setFont(
                            QFont(
